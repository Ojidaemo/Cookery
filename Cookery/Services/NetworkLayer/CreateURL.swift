...
extension CreateURL {
    ...
    var url: URL {
        ...
        case .randomURL(let apiKey):
            if let url = URL(string: "https://api.spoonacular.com/recipes/random?apiKey=\(apiKey)&number=3") {
                return url
            }
            fatalError()
        case .categoryURL(let apiKey, var type):
            type = type.replacingOccurrences(of: " ", with: "+")
            if let url = URL(string: "https://api.spoonacular.com/recipes/complexSearch?type=\(type)&apiKey=\(apiKey)") {
                return url
            }
            fatalError()
